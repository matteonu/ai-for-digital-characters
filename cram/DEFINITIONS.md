...
**HRV:** variability of the time between consecutive heartbeats (R-R intervals) — reflects autonomic nervous system activity; higher HRV generally = relaxed, lower = stress.
...
**Why RNNs suit sequential data (1 pt):** they maintain a hidden state passed from step to step, so each output depends on all previous inputs, and the same weights are shared across time steps (works for variable-length input).
...
**Backprop trap (2025 MC):** gradients **do** depend on the activation functions — the chain rule multiplies their derivatives.
...
**Power spectrum meaning:** the energy present in each frequency bin of a frame.
...
**Mel scale:** perceptual, **logarithmic** frequency scale — human pitch resolution is finer at low frequencies. Mel = 1127·ln(1 + f/700).
...
**top-k vs top-p (2 pts):** top-k always keeps a **fixed number** k of the highest-probability tokens; top-p keeps the **smallest set whose cumulative probability ≥ p**, so the candidate set **adapts** to how peaked or flat the distribution is.
...
**Prosody — two phenomena:** **stress/accent** (which syllable or word is prominent; a **nuclear accent** is the last and most prominent accent in a phrase) and **intonation** (the F0 contour, e.g. rising for a question), plus phrasing/pauses and duration.
- 2025 MC: **stress depends on sentence context, accent is fixed within the word.**

**Unit selection:** pick database units minimizing **target cost** T(sₜ,uⱼ) — how well a unit matches the desired description (F0, stress, duration, position) — plus **join cost** J(uⱼ,uⱼ₊₁) — how smoothly it joins the next unit (F0, energy, spectral features). Join cost is **0** for units that were consecutive in the original recording. Limitation: quality depends on database coverage; modern neural TTS removes this.
...
**Vocoders:** **Griffin-Lim** iteratively reconstructs **phase** from the magnitude spectrogram — fast, lowest quality. **WaveNet** uses **causal dilated convolutions**, autoregressive — excellent quality but slow sample-by-slow-sample inference. **HiFi-GAN** is GAN-based and parallel — **best choice for real-time**, high quality and fast.
...
represent each anchor entity as a **zero-volume box** (a point); apply a **projection** operator P(box, relation)
per relation hop, which **expands** the box to cover all entities reachable by that relation; then apply the
**intersection** operator I(box₁, …, boxₙ) for the AND — its centre is a weighted sum of the input centres and
its extent is the geometric intersection. Entities falling **inside the final box** are the answers.
...
**Why TransE fails on symmetric relations:** both (h,r,t) and (t,r,h) true ⇒ h + r ≈ t and t + r ≈ h. Adding gives 2r ≈ 0 ⇒ **r ≈ 0**, which forces **h ≈ t** — the entities collapse to the same embedding and become indistinguishable.
...
- **State sₜ:** what the agent observes now (e.g. position + velocity, or agent position + goal position).
- **Action aₜ:** what it can do (e.g. movement direction/velocity, joint torques).
- **Policy π(a|s):** the mapping from state to action, here an MLP; for a **stochastic** policy the network outputs a **distribution** over actions (e.g. mean and variance of a Gaussian, or softmax over discrete actions) that you sample from.
- **Reward rₜ:** scalar feedback (e.g. progress toward goal minus a control/collision penalty).
...
**Q-learning:** the policy is **implicit** — take the action with the largest Q value. *(Slides mark this "for self-study" — low priority.)*

---

## Q9 — Animation

**FK vs IK:** forward kinematics maps **joint angles → end-effector position**; inverse kinematics finds the **joint angles that reach a target** position.
...
**Manifold learning (2 pts):** human motion is highly constrained, so it lies on a low-dimensional manifold. Manifold learning finds the small set of intrinsic variables (joint angles, speed) that explain the data via f: ℝᵈ → ℝᵐ, making synthesis and interpolation tractable and dodging the curse of dimensionality. *Application: motion synthesis / interpolation between motions.*
...
**Normalizing flows (2 pts):** describe a complex distribution as a chain of **simple, invertible, differentiable** transformations of a simple prior (x = f₁(f₂(…f_N(z)))). Because each step is invertible you can both **sample** and **evaluate the exact probability density** — combining the benefits of generative models and explicit PDF modelling. *Application: generating diverse character motion.*
